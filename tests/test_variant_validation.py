...
def test_variant_add_success(client, logged_in_staff, test_product_for_variant):
    """測試成功新增變體與多個 SKU"""
...
def test_variant_add_duplicate_sku(client, logged_in_staff, test_product_for_variant):
...
def test_variant_add_negative_values(client, logged_in_staff, test_product_for_variant):
    """測試提交負數值"""
    with patch('app.blueprints.staff.permission.check_permission', return_value=True):
        response = client.post(f'/staff/product/{test_product_for_variant}/variant/add', data={
            'color': '綠色',
...
def test_variant_edit_duplicate_sku_database(client, logged_in_staff, test_product_for_variant):
